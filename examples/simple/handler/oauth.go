...
func GetAccessToken(ctx *contx.Context) {
	...
	auth.RemoveUnusedAC(contatoID)
	auth.AccessTokenCache[acID] = ac
	go repo.AddAccessTokenRequestLog(acID, ac.ContatoID)
	acPub := model.AccessTokenPublic{}
...
func GetUserCredentials(ctx *contx.Context) {
	...
	user.Name, err = repo.GetUserNameByContactID(user.UserID)
	if err != nil {
	...
	hasRole, hasCredentials, err := repo.StatusUserCredentials(user)
	if err != nil {
		log.Printf("[GetUserCredentials] Erro ao verificar se o usuario [%d] tem credenciais. Erro: %s\n", user.UserID, err.Error())
		ctx.JSON(http.StatusInternalServerError, "{'error': 'Could not generate credentials'}")
		return
	}
	if !hasCredentials {
		clientID, secret, err := generateUserCredentials(user, ctx.Req.RemoteAddr)
		if err != nil {
			log.Printf("[GetUserCredentials] Erro ao gerar o clientID e secret para o usuario [%d]. Erro: %s\n", user.UserID, err.Error())
			ctx.JSON(http.StatusInternalServerError, "{'error': 'Could not generate credentials'}")
			return
		}
		if !hasRole {
			err = repo.AddCredentialsToUser(user, clientID, secret)
			if err != nil {
				log.Printf("[GetUserCredentials] Erro ao salvar as credenciais do usuario [%d] no banco. Erro: %s\n", user.UserID, err.Error())
				ctx.JSON(http.StatusInternalServerError, "{'error': 'Could not generate credentials'}")
				return
			}
		} else {
			err = repo.UpdateUserCredentials(user, clientID, secret)
			if err != nil {
				log.Printf("[GetUserCredentials] Erro ao atualizar as credenciais do usuario [%d] no banco. Erro: %s\n", user.UserID, err.Error())
				ctx.JSON(http.StatusInternalServerError, "{'error': 'Could not generate credentials'}")
				return
			}
		}
	...
	clientID, secret, err := repo.GetUserCredentials(user)
	if err != nil {
...
func generateUserCredentials(user model.OAuthUser, remoteAddr string) (clientID string, secret string, err error) {
	err = nil
